...
class Calculator:
    def __init__(self, master):
        ...
        self.entry_value = ''
        Entry(width=43, bg='white', font=('Times', 16), textvariable=self.equation).place(x=0, y=0)
        ...
        Button(width=8, text='9', relief='flat', command=lambda: self.show(9)).place(x=180, y=170)
        Button(width=8, text='8', relief='flat', command=lambda: self.show(8)).place(x=90, y=170)
        ...
        Button(width=8, text='Kok', relief='flat', command=self.kok).place(x=180, y=50)
        Button(width=8, text='Us', relief='flat', command=lambda: self.show('**')).place(x=90, y=50)
        Button(width=8, text='=', bg='white', relief='flat', command=self.solve).place(x=180, y=210)
        Button(width=8, text='AC', relief='flat', command=self.clear).place(x=270, y=50)

    def show(self, value):
        self.entry_value = str(self.entry_value) + str(value)
    ...
    def kok(self):
        result = eval('math.sqrt(int(self.entry_value))')
